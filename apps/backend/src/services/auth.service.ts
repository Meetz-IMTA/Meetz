...
export const registerUser = async (data: {
  ...
  const accessToken = jwt.sign({ userId: savedUser.id }, ACCESS_SECRET, { expiresIn: '15m' });
  const refreshToken = jwt.sign({ userId: savedUser.id }, REFRESH_SECRET, { expiresIn: '7d' });
  ...
  const { password: _, refreshToken: __, ...userWithoutPassword } = verifiedUser;
  return { email: data.email, accessToken, refreshToken, user: userWithoutPassword };
};
...
export const verifyOtpService = async (email: string, otp: string) => {
  ...
  if (!user.otpExpiry || user.otpExpiry < new Date()) throw new Error('Code expiré.');
...
export const loginUser = async (data: {
  ...
  if (!user) throw new Error('Utilisateur non trouvé');
  if (!user.isVerified) throw new Error('Compte non vérifié. Vérifie ton email.');
